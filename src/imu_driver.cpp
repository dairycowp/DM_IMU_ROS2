...
void DmImu::publish_imu_data()
{
    ...
    imu_msg->angular_velocity_covariance[0] = 0.000025;
    imu_msg->angular_velocity_covariance[4] = 0.000025;
    imu_msg->angular_velocity_covariance[8] = 0.000025;
...
{
    // 设置输出频率为1000Hz
  const uint8_t txbuf[5]={0xAA, 0x02, 0x05, 0x00, 0x0D};
  for(int i=0;i<5;i++)
  {
    serial_imu.write(txbuf,sizeof(txbuf));
    rclcpp::sleep_for(std::chrono::milliseconds(10));
